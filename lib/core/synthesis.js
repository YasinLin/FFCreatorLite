...
class Synthesis extends FFBase {
   ...
   */
  addXfadeInput() {
    const { scenes, subtitle } = this.creator;
    const filters = [];
    let offset = 0;
    let cid;
    let last;
    ...
    if (filters.length) this.command.complexFilter(filters, cid);
  }
  ...
  addAudio() {
    ...
    let args = conf.getVal('audio');
    if (isEmpty(args)) return;

    args = typeof args === 'string' ? { path: args } : args;
    if (args.path) command.addInput(args.path);
    if (args.loop === undefined) args.loop = true;
    args.loop && command.inputOptions(['-stream_loop', '-1']);
  }
