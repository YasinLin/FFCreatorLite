...
class Renderer extends FFBase {
  ...
  async synthesisOutput() {
    ...
    this.synthesis = new Synthesis(creator);
    this.synthesis.setDuration(creator.getScenesDuration());
    this.synthesis.addAudios(audios);
    this.synthesis.on('synthesis-error', this.eventHandler.bind(this));
    this.synthesis.on('synthesis-complete', event => {
      Perf.end();
      event.useage = Perf.getInfo();
      event.percent = 1;
      this.emit('all-complete', event);
    });
    this.synthesis.on('synthesis-progress', event => {
      event.type = 'progress';
