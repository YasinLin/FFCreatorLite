...
const FilterUtil = {
  ...
  createSubtitleFilter(path, styles, inputs, outputs) {
    styles = Utils.deleteUndefined(styles);
    return {
      filter: 'subtitles',
      options: path,
      force_style: styles,
  ...
  setInputsAndOutputs({ filter, contextInputs, inputs, outputs }) {
    if (typeof filter === 'string') {
      ...
      if (/^overlay=/g.test(filter)) {
        ...
        if (inputs) filter = `[${inputs}]${filter}[${outputs}]`;
        else filter = `${filter}[${outputs}]`;
      }
    } else {
      // is overlay filter
      if (filter.filter === 'overlay') {
        ...
        if (inputs) filter.inputs = inputs;
        filter.outputs = outputs;
      }
    }
