...
const FFmpegUtil = {
  ...
  async getFfmpegInfo(videoPath, cache_dir) {
    return new Promise((resolve, reject) => {
      // 获取视频封面图、时长、大小、宽度和高度信息
      ffmpeg.ffprobe(videoPath, (err, metadata) => {
        if (err) {
          ...
          const width = metadata.streams[0].width; // 视频宽度
          const height = metadata.streams[0].height; // 视频高度
          const filename = utils.uid() + '.png';
          const cover = _path.resolve(cache_dir, filename);
          ...
          ffmpeg(videoPath)
            ...
            .on('end', () => {
              console.log(
                'Video Thumbnail, Duration, Size, Width and Height information successfully retrieved',
              );
              resolve({
                duration,
                size,
