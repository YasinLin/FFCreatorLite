...
class FFExector extends EventEmitter {
  ...
  async parse_slide_panel(p_creator, m_width, m_height, scene, layer, time) {
    ...
    const debug = p_creator.rootConf('debug');
  ...
  async parse_subtitle(creator, m_width, m_height, scene, subtitle) {
    ...
    const width = m_width;
    const height = m_height;
    const { audio, texts, object, fontPath, enable = true } = subtitle;
  ...
  async parse_clip(creator, m_width, m_height, clip, time) {
    ...
    //转场
    if (transition_name) {
      scene.setTransition(transition_name, transition.duration || 1);
    }
    ...
    if (subtitle && subtitle.texts && subtitle.texts.length > 0) {
      ...
      if (ffsubtitle) scene.addChild(ffsubtitle);
    }
  ...
  async sync(json) {
    ...
    creator.on('progress', e => {
      ...
      if (creator.getOutput()) {
        this.setVideo(creator.getOutput());
      }
    });
