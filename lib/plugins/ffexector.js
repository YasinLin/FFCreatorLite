...
class FFExector extends EventEmitter {
  ...
  async parse_video(creator, m_width, m_height, scene, layer, time) {
    ...
    // 幻灯片播放完成后再执行
    fvideo.setAppearTime(cutTo - cutFrom);
    // });
  ...
  async parse_image(creator, m_width, m_height, scene, layer, time) {
    ...
    switch (resizeMode) {
      ...
      case 'contain-blur':
        ...
        fimg1 = new FFImage({
          path: file,
          x: m_width / 2 - keep_wh_cover.w / 2,
          y: m_height / 2 - keep_wh_cover.h / 2,
          opacity: 0.5,
          ...keep_wh_cover,
          resetXY: false,
        });
    ...
    const x = m_width / 2 - keep_wh.w / 2;
    const y = m_height / 2 - keep_wh.h / 2;
    const fimg = new FFImage({
      path: file,
      x,
      y,
  ...
  async sync(json) {
    ...
    creator.on('start', async () => {
      this.emit('start');
      await this.setProgress(creator.id, 0);
    });
    ...
    creator.on('progress', async e => {
      const percent = e.percent || 0;
      await this.setProgress(creator.id, percent);
      e.percent = await this.getProgress();
      this.emit('progress', e);
