...
class FFScene extends FFCon {
  ...
  async deleteCacheFile() {
    ...
    if (!debug && this.directory) await rmfr(this.directory);
  }
  ...
  toFiltersCommand(command) {
    ...
    if (filters.length) command.complexFilter(filters, this.context.input);
  }
  ...
  async destroyChilds() {
    await Promise.all(this.children.map(async child => await child.destroy()));
  ...
  async destroy() {
    ...
    await this.destroyChilds();
    await FFmpegUtil.destroy(this.command);
    await super.destroy();
    if(this.transition) await this.transition.destroy();
    this.transition = null;
    this.context = null;
    this.command = null;
  }
